...
gui::TextField::TextField(const std::function<void(const std::string&)>& processTextInputFunction, const unsigned short fieldWidth, const sf::Font& font, const unsigned char characterSize)
...
gui::TextField::TextField(const TextField& source)
...
const bool gui::TextField::input(const sf::Event& event)
{
	switch (event.type)
	{
...
void gui::TextField::processCurrentInput()
{
...
const sf::Vector2f& gui::TextField::getPosition() const
{
	// TODO: insert return statement here
}

const sf::FloatRect gui::TextField::getGlobalBounds() const
{
	return sf::FloatRect();
}

const std::string& gui::TextField::getCurrentInput()const
{
	return currentInput.getString();
}

const unsigned char gui::TextField::getCharacterSize()const
{
	return currentInput.getCharacterSize();
}

const sf::Color& gui::TextField::getTextColor()const
{
	return currentInput.getColor();
}

gui::TextField& gui::TextField::setCharacterSize(const unsigned char characterSize)
{
...
gui::TextField& gui::TextField::setTextColor(const sf::Color& color)
{
	currentInput.setColor(color);
	return *this;
}

gui::TextField& gui::TextField::setPrompt(const std::string& str, const sf::Color& color)
{
...
gui::TextField& gui::TextField::setPromptColor(const sf::Color& color)
{
	if (!prompt) prompt.reset(new sf::Text("", *currentInput.getFont(), currentInput.getCharacterSize()));
	prompt->setColor(color);
}

gui::TextField& gui::TextField::clearPrompt()
{
	prompt.reset();
	return *this;
}

gui::TextField& gui::TextField::setPosition(const float x, const float y)
{
	position.x = x;
	position.y = y;
...
gui::TextField& gui::TextField::setFont(const sf::Font& font)
{
...
gui::TextField& gui::TextField::setFieldWidth(const unsigned short width)
{
	box.reset(sf::FloatRect(0, 0, width, box.getSize().y));
	return *this;
}

gui::TextField& gui::TextField::setClipboardPermission(const bool permission)
{
